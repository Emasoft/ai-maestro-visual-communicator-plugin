...
Every page generated by `ai-maestro-visual-communicator` is interactive: the user clicks an element (or fills a form built from a table), the selection is sent back to the agent, and the browser window closes automatically. The agent then prompts the user about what to do with the selection.
...
| `finding-reply`   | (no prompt — read the user's per-finding text and append a per-finding `claude` response to the sidecar replies.json, then re-render via `/aimvc-interactive-report`) |
