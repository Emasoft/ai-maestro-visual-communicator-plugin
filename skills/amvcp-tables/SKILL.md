---
name: amvcp-tables
description: "Render data, matrix/checklist and comparison tables — click-to-sort, window-scroll virtualization with frozen header/columns, status-glyph coverage grids, side-by-side comparison with emphasis column, copy-as-CSV. Per-row decoration vocabulary (risk dot, severity badge, stat card, kanban-as-table, decision matrix). Use when the user wants tabular data, audits, comparisons, triage lists. Trigger with 'table', 'sortable table', 'data table', 'feature matrix', 'comparison table', 'risk table', 'severity badge', 'kanban triage', 'decision matrix', 'export CSV'."
license: MIT
