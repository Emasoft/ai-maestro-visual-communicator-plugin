...
description: "Code-as-data and editable-fence surface — distinguishes the pre tag as code vs data (CSV, JSON, YAML), the data-ve-no-gutter opt-out attribute for blocks that must skip the gutter (regex graph, overlay snippets), and the contenteditable code-editor family (caret-save/restore via TreeWalker, Enter-intercept, paste-as-plain, RAF-debounced re-highlight, JSON live-validate). Use when scaffolding a CSV/JSON data block, a live editor, an opt-out gutter block, or deciding fenced vs free-form. Trigger with 'CSV', 'JSON data', 'YAML data', 'data fence', 'opt-out gutter', 'contenteditable', 'JSON editor', 'live edit code', 'no-gutter'."
license: MIT
