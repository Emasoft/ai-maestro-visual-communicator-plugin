...
| "Form with many fields" | any | [`form-patterns.md`](references/form-patterns.md) |
  > Overview · Overview · Single-column form (the default) · Two-column form (wide screens) · Field group — related inputs share a section · Input variants — textarea, select, file, multi-line · Inline help — `wf-label` + small text · Error state — invalid input + error message · Success state — saved confirmation · Required-field marker · Optional-field marker (inverted convention) · Multi-step wizard (progress bar + per-step forms) · Login + signup pair (the auth duo) · Search form (single input + filter row) · Action bar — sticky save / cancel at the bottom
