...
| Horizontal scrollbar on a `<pre>` | A CSS rule set `overflow-x:auto` — remove it. The page expands; wide code wraps. See [wrap-and-no-inner-scroll](../../amvcp-code-syntax-engine/references/wrap-and-no-inner-scroll.md) |
| Colors wrong on light theme | `:root[data-ve-theme="light"]` mirror is missing for one or more `--ve-code-*` tokens. See [light-dark-mirror-discipline](../../amvcp-code-syntax-chrome/references/light-dark-mirror-discipline.md) |
