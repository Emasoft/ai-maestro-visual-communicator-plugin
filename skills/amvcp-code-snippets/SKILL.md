...
1. **Pick the composition pattern** from the Resources index below — each entry carries the COMPLETE TOC of its reference file. Each ref is self-sufficient; the markup is compositional but the patterns are codified.
2. **Collapsed walkthroughs.** `<details>` per step, mutually-exclusive disclosure via `toggle` event (one open at a time), file:line summary in the `<summary>`, hot-step modifier for the active step. See [collapsed-snippets-walkthrough](references/collapsed-snippets-walkthrough.md) (TOC in Resources).
3. **Tabbed multi-perspective.** 6-line JS tabbar, `[button.on data-t="0"]` ↔ `[<pre>.on]` toggle, 3-perspectives-on-the-same-change pattern. See [tabbed-code-panel](references/tabbed-code-panel.md) (TOC in Resources).
4. **Click-step to code panel.** SVG flowchart node click → sticky right-side detail panel with `<pre>` code excerpt, `DETAIL = { k: { title, meta, body, code } }` map. See [click-step-to-code-panel](references/click-step-to-code-panel.md) (TOC in Resources).
5. **Code snippet on diagram hover.** Hover any chart bar / diagram node → JSX prop snippet renders below with live token values interpolated. See [code-snippet-on-diagram-hover](references/code-snippet-on-diagram-hover.md) (TOC in Resources).
6. **Architecture / feature explainer.** Long-form composition pages with sticky TOC + step `<details>` + tabbed config + provenance footer. See [architecture-explainer-snippets](references/architecture-explainer-snippets.md) and [feature-explainer-tabbed](references/feature-explainer-tabbed.md) (TOCs in Resources).
7. **Multi-file plans / comparisons.** 2-col implementation-plan grid, N-column compare-approaches with Pro/Con + metrics. See [implementation-plan-codepanels](references/implementation-plan-codepanels.md) and [compare-n-approaches](references/compare-n-approaches.md) (TOCs in Resources).
...
Two copy-paste skeletons (tabbed multi-perspective panel; collapsed
one-open-at-a-time walkthrough) live in
[composition-examples](references/composition-examples.md).
  > Composition examples · Example 1 — tabbed multi-perspective code panel · Example 2 — collapsed walkthrough (one step open at a time)
...
- [composition-examples](references/composition-examples.md) — the two minimal markup skeletons (tabbed multi-perspective panel; collapsed one-open-at-a-time walkthrough).
  > Composition examples · Example 1 — tabbed multi-perspective code panel · Example 2 — collapsed walkthrough (one step open at a time)
