...
- **[amvcp-code-syntax](../amvcp-code-syntax/SKILL.md)** — the substrate umbrella; routes to two focused sub-skills (load the umbrella first, then the sub-skill(s) you need):
  - **[amvcp-code-syntax-engine](../amvcp-code-syntax-engine/SKILL.md)** — tokenizer + 12-token palette + per-line gutter + copy button + drag-paint selection + integrity probe. 8 reference files.
  - **[amvcp-code-syntax-chrome](../amvcp-code-syntax-chrome/SKILL.md)** — visual chrome (block 3-state, blueprint theme, slate-bg, file-path / tab-bar headers, inline chip, hand-wrap palette, keyword highlight) + cross-cutting discipline (author-vs-runtime boundary, light-dark mirror). 10 reference files.
