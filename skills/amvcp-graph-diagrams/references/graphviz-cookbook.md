...
How to render directed-graph and graph-theoretic figures in pages generated by `ai-maestro-visual-communicator-plugin` via the `.ve-graph` (Graphviz / viz.js) engine. For the cross-cutting selection wire format, read `${CLAUDE_PLUGIN_ROOT}/references/interactive-selection-base.md` first. For the engine-routing decision (when to use Graphviz vs. Mermaid vs. TikZ vs. Chart.js), see the "Engine routing" section in that base file.
