...
[multi-column-layout.md](./multi-column-layout.md)) is the home for
...
- [lead-paragraph.md](./lead-paragraph.md) — lead paragraphs in
  narrow columns also benefit from `.vc-type-justify` (often paired
  with `.vc-type-lead.vc-type-justify`).
- [multi-column-layout.md](./multi-column-layout.md) — the
  `.vc-type-multicol` utility where `widows` / `orphans` take effect.
- [language-and-locale.md](./language-and-locale.md) — the `<html
