...
- The **`vercel-deploy`** skill installed via `pi install npm:vercel-deploy`. `share.py` only searches the Pi skill search paths, so it must be a Pi-compatible install (a plain `vercel` CLI is not detected).
