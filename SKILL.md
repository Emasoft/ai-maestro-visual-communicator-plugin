...
**Math and code blocks inside `[data-ve-prose]` follow the same depth pattern.** A click on a `.ve-math` formula walks atom → group → whole formula at depths 1-3 (KaTeX class names) then climbs the prose hierarchy at 4-7 — emits `kind:"math"` with `formulaLatex` (the parent `.ve-math` LaTeX source) and the surrounding paragraph's pnum/text. A click on a `<pre>` walks token → line → whole block at depths 1-3 (Prism `.token` ancestor or word-range fallback) then climbs the prose hierarchy at 4-7 — emits `kind:"code"` with `language` (from `data-language` or `<code class="language-X">`) and the closest preceding `[data-ve-pnum]` for context, since `<pre>` isn't auto-numbered. See `./references/interactive-selection.md` "Phase 3 — math grammar" and "Phase 3 — code grammar" for full depth tables and the `[data-ve-math-sel]` / `[data-ve-code-sel]` highlight CSS.
...
**AI-generated illustrations (optional).** If [surf-cli](https://github.com/Emasoft/surf-cli) is available, you can generate images via Gemini and embed them in the page for creative, illustrative, explanatory, educational, or decorative purposes. Check availability with `which surf`. If available:
...
```bash
bash ~/.pi/agent/skills/ai-maestro-visual-communicator/scripts/share.sh <html-file>
```

If the skill lives somewhere else, use that install path instead, such as `~/.codex/skills/ai-maestro-visual-communicator/scripts/share.sh`, `~/.config/opencode/skill/ai-maestro-visual-communicator/scripts/share.sh`, or `./scripts/share.sh` from a repository checkout.

**Example:**
```bash
bash ~/.pi/agent/skills/ai-maestro-visual-communicator/scripts/share.sh ~/.agent/diagrams/my-diagram.html
...
**How it works:**
1. Runs the `share.sh` script from the installed `ai-maestro-visual-communicator` skill directory
