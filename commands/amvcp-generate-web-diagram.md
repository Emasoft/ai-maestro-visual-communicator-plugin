...
Mark every meaningful element selectable with `data-ve-id` / `data-ve-type` / `data-ve-label`, embed `<script src="amvcp-runtime.js"></script>` at end of body, and add `click X call veSelectMermaid("X","Label")` directives plus `securityLevel: 'loose'` to any Mermaid diagrams. See `./references/interactive-selection.md` for the full pattern.
